function SBMLtoMATLAB(SBML_filename, matlabFilename)
  % REQUIRES SBML  and MATLAB-SBML DOWNLOADS
  % See https://sbml.org/software/sbmltoolbox/ for details
    model = TranslateSBML(SBML_filename);

    WriteODEFunction(model, matlabFilename);
end

function WriteODEFunction(varargin)
    %
    % MODIFIED BY NATHANIEL LINDEN
    %
    %
    %
    %
    ...
    % NJL - EDIT Aug 5 2022
    % 
    % Modifed to be in format for ode solver
    % odeFunc(x, params)
    %
    ...
    % if no events and using octave
    if (exist('OCTAVE_VERSION') && NumEvents == 0)
      fprintf(fileID,  'function xdot = %s(x_values, p, %s)\n', Name, timeVariable);
    else
      fprintf(fileID,  'function xdot = %s(%s, x_values, p)\n', Name, timeVariable);
    end;
    ...
    % NJL - EDIT Aug 5 2022
    % 
    % Modifed to set parameter values to intput p
    %
