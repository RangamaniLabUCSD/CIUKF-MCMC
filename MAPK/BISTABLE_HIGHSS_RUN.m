...
f = @(k, x, theta)propf(x,@(t, x) MAPK(t, x,theta), dt, k, @(t, x) Jac(t, x,theta));

% Initialize noise
% original multiplier for results_HIGHSS_run1.mat
% c = 0.1; % multiplier
% high noise multiplier for results_HIGHSS_NOISY1.mat
c = 0.5; % multiplier
sigmaR = c*std(yTruehigh)'; % measurement noise

%%%% HIGH STEADY STATE! %%%%%
% Generate noisy samples
y = generateData(@(x) MAPK(x, ptrue), x0high, t, H, sigmaR, odeOptsTrue); % Use ode45 to solve the system and get 'training data'
...
logLikelihood = @(theta, y) ciukflp_quadProg(theta, x0high, P0, f, H,...
...
save([savedir, 'results_HIGHSS_noisy1.mat'], 'BayesianAnalysis')

% RUN WITH AIES %
%%%% ADDITIONAL RUN %%%%%
% load previous run
prev = load([savedir, 'results_HIGHSS_noisy1.mat']);
